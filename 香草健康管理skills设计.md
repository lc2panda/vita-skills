...
本产品遵循 Comdr 提出的六项设计哲学，作为整体架构决策的指导原则：

**哲学 1：Vibe Coding 心流场景适配**
用户正在 Claude Code 等工具中进行 Vibe Coding，处于高度专注的心流状态。提醒需要适时介入但不能粗暴打断。需要"强提醒"机制——AI 可在后台 autopilot 托管干活，让用户暂停释放。设计目标：心流检测 → 渐进式提醒 → AI 托管过渡。详见 §5.5。

**哲学 2：用眼提醒频率折中调整**
原始 25 分钟间隔过于频繁。根据超日节律（ultradian rhythm）研究，人类注意力自然周期约 90 分钟 [DR1]，一个完整注意力子周期约 45-50 分钟 [DR2]。设计需在眼科医学证据（20-20-20 规则）与注意力科学之间折中。详见 §3.2.1 间隔参数对比分析。

**哲学 3：动态自适应提醒机制**
不能机械式固定间隔提醒。需评估用户历史执行情况，动态分配提醒频率：执行好的用户可降低频率，跳过提醒的用户加强提醒。基于 JITAI（Just-in-Time Adaptive Intervention）框架 [DR3]。详见 §5.7。

**哲学 4：多渠道提醒同步推送**
如 Claude Code 安装了 channel 消息渠道（如 WeChat channel、飞书 channel），提醒需同步推送到对应 channel。基于 Claude Code Channels 机制实现多 channel 同步 [DR4]。详见 §5.6。

**哲学 5：用户执行忠诚度评估**
需要一个机制评估用户对提醒的执行情况。设计忠实度评分模型（打卡率、响应时间、连续执行天数、跳过模式），分为模范/良好/待改善/流失风险四级，与打榜权重联动。详见 §5.7.2。

**哲学 6：打榜昵称合规校验**
安装阶段用户输入昵称需合法性校验：政治人物人名校验 + 其他非法内容校验 + AI 合规性审查。唯一性通过用户名 + 机器特征绑定保证。详见 §4.9。
...
#### 3.2.1 间隔参数折中分析（设计哲学 2）

原始设计采用 25 分钟间隔，但此频率在 Vibe Coding 场景下过于频繁，频繁打断心流。当四条提醒管线同时运行时，用户感知到的打断密度过高。以下为四种候选间隔的对比分析：

| 方案 | 间隔 | 每天提醒次数 (8h) | 科学依据 | 心流影响 | 眼保护效果 |
|------|------|-----------------|---------|---------|-----------|
| A: 原始 | 25 min | ~19 次 | 传统 Pomodoro (Cirillo, 1980s)；AOA/CAO 20-20-20 规则 [EC7:EC8] | **严重打断**：心流恢复平均需 23 分钟 [DR5] | 强（但执行率仅 34% [EC5]） |
| B: 半超日 | 45 min | ~11 次 | 1 个注意力子周期约 45-50min [DR2]；每 45-60min 中断久坐亦可改善代谢 [SD3] | 中等影响：与自然注意力衰减点对齐 | 中等 |
| C: 整周期 | 50 min | ~10 次 | 60-60-30 时间管理法 (WU Vienna) [DR2]；注意力持续约 50min 后自然下降 | 较低影响：匹配注意力自然节律 | 中等 |
| D: 超日节律 | 90 min | ~5 次 | Kleitman 超日节律 (1960s) [DR1]；精英表现者实践 (Ericsson, 1993)；大脑钠钾泵恢复周期 | **低影响**：与完整工作周期对齐 | 弱：远低于 20-20-20 的证据阈值 |

**推荐折中方案：50 分钟默认 + 25/45/90 可选**

核心逻辑：
1. **不采用 25 分钟**：眼科学证据虽强，但执行率仅 34% [EC5]；在 Vibe Coding 场景下与心流保护冲突严重
2. **不采用 90 分钟**：对用眼保护太弱，远低于 20-20-20 的证据阈值
3. **50 分钟为折中最优**：匹配人类注意力自然子周期（约 45-50 分钟）[DR2]，在打断次数（~10 次/天）与眼保护效果之间取得平衡。一个注意力周期结束时，用户正处于自然认知衰减点，此时提醒的"打断成本"最低
4. **配套补偿**：休息时长由 30 秒延长至 60 秒，以补偿频率降低带来的总休息时间减少（50min x 60s vs 25min x 30s → 每天总休息时间从约 9.5 分钟增至约 10 分钟）

| 参数 | 默认值 | 可调范围 | 证据 |
|------|--------|---------|------|
| 提醒间隔 | **50 分钟** | 25/45/50/90 分钟 | 超日节律 [DR1] + 注意力子周期 [DR2] + AOA/CAO [EC7:EC8] 折中 |
| 休息时长 | **60 秒** | 30 秒/60 秒/90 秒 | 改良 20-20-20-20 规则 [EC4] + 频率降低补偿 |
...
安装阶段用户输入昵称时，需要多层校验确保合法性、合规性和唯一性。
...
第 1 层：长度与字符校验（本地）
    |-- 最短 1 字符，最长 20 字符
    |-- 允许：中日韩字符、拉丁字母、数字、下划线、短横线
    |-- 禁止：纯数字、纯符号、空白字符开头/结尾
    |
    v
第 2 层：敏感词库匹配（本地 + 远程）
    |-- 政治人物人名库（含拼音/英文别名/缩写）
    |-- 通用敏感词库
    |-- 正则模式匹配（变体绕过检测，如全角/半角混用）
    |
    v
第 3 层：AI 合规审查（远程 API）
    |-- LLM 判定：昵称是否隐含政治人物/事件/敏感内容
    |-- 语境感知：检测暗示性、影射性表达
    |-- 返回：通过/拒绝 + 拒绝理由
    |
    v
第 4 层：唯一性校验（本地 + 远程）
    |-- 设备指纹生成（见 4.9.3）
    |-- 查重：username + device_fingerprint 组合绑定
    |-- 允许同一设备更换昵称（更新绑定），不允许不同设备使用同一昵称
    |
    v
通过 -> 注册成功
拒绝 -> 返回具体拒绝原因 + 建议
...
| 数据源 | 覆盖范围 | 维护策略 |
|--------|---------|---------|
| Wikipedia "List of current heads of state and government" | 全球 196 国现任及近期离任（10 年内）国家元首/政府首脑 | 每周自动抓取更新 |
| 中国政治人物名录 | 现任及历届（1949 年起）党和国家领导人、省部级主官 | 手动维护 + 每季度审查 |
| 历史政治敏感人物 | 国际公认的争议政治人物名单 | 手动维护 |
| 别名/外号映射 | 每个政治人物的常见别名、外号、缩写、拼音变体 | 众包 + LLM 生成验证 |

**变体防绕过策略** [DR10]：
- Unicode 同形字检测（如 Cyrillic 'a' vs Latin 'a'）
- 全角/半角字符标准化
- Levenshtein 距离 < 2 的近似匹配
- 拼音变体与英文缩写匹配

#### 4.9.3 设备指纹生成规则

为保证昵称唯一性绑定，生成设备指纹，规则如下 [DR11]：

| 指纹因子 | 熵值估计 | 说明 |
|---------|---------|------|
| OS 平台 + 版本 | ~5 bits | 如 darwin-25.5.0 |
| 主机名哈希 | ~8 bits | SHA-256 前 8 字符 |
| Claude Code 安装路径 | ~3 bits | 固定路径值 |
| 用户主目录路径哈希 | ~8 bits | SHA-256 前 8 字符 |
| 机器 UUID (macOS) / MachineGuid (Windows) | ~16 bits | 系统唯一标识 |

**组合熵**：~40 bits，对于 < 1,000,000 用户规模的唯一性需求足够 [DR11]。

**隐私处理**：
- 所有因子在本地计算 SHA-256 哈希后上传，不传输原始值
- 设备指纹仅用于唯一性绑定，不关联个人身份信息
- 用户更换设备时提供"迁移绑定"机制（旧设备确认 + 3 天冷却期）

#### 4.9.4 AI 合规审查 Prompt 设计

```
System: 你是内容合规审查员。审查用户提交的昵称。
拒绝以下情况：
1. 包含或暗示政治人物姓名（现任/历届，国内外）
2. 包含违法、暴力、仇恨、色情内容
3. 包含广告、钓鱼、欺诈意图
4. 冒充官方机构或公众人物
...
{
  "verdict": "pass" | "reject",
  "reason": "简洁的拒绝理由（中文，20字以内）",
  "suggestions": ["建议1", "建议2"]  // 仅 reject 时
}
```

**审查原则**：
- 宽松通过，严格拒绝——仅拦截明显违规内容
- 模糊边界（如常见姓氏与政治人物同字）默认为通过
- 拒绝必须给出具体可操作的建议
...
### 5.5 心流适配与强提醒机制（设计哲学 1）

Vibe Coding 场景下的核心挑战：用户在 Claude Code 等工具中处于高度专注的心流状态，粗暴的弹窗提醒会打断思维链，心流恢复平均需要约 23 分钟 [DR5]。需要设计一种既不破坏心流，又能确保健康提醒被执行的机制。

#### 5.5.1 心流检测策略

通过以下多维信号综合判定用户是否处于心流状态：

| 检测维度 | 信号源 | 心流判定指标 | 权重 |
|---------|--------|------------|------|
| 输入活跃度 | 键盘/鼠标事件（IDE/终端 hook） | 过去 3 分钟内持续输入，停顿 < 5 秒 | 0.35 |
| 代码变更速率 | git diff 行数变化率 | 过去 5 分钟 > 10 行有效代码变更 | 0.25 |
| IDE 窗口焦点 | OS 窗口焦点事件 | 终端/IDE 持续前台 > 5 分钟 | 0.20 |
| AI 对话活跃度 | Claude Code 会话频率 | 过去 3 分钟 >= 2 次对话交互 | 0.10 |
| 音乐/噪音背景 | 系统音频输出状态 | Lo-fi/白噪音播放中（可选信号） | 0.10 |

心流判定公式：加权得分 >= 0.65 视为进入心流状态 [DR6]。

#### 5.5.2 渐进式提醒策略

| 心流状态 | 提醒等级 | 行为 |
|---------|---------|------|
| 未检测到心流 | 常规提醒 | 标准弹窗 + 声音 + channel 推送（如有） |
| 心流中（<30 分钟） | 温和提醒 | Toast 通知（3 秒自动消失），不打断输入 |
| 心流中（30-60 分钟） | 渐进提示 | 终端底部状态栏文字提醒，持续显示直到确认 |
| 心流中（>60 分钟） | 强提醒晋级 | 半透明 overlay + 10 秒后语音播报，主动暂停当前 AI 操作 |

晋级逻辑：心流持续时间越长，提醒强度递增，确保不无限期推迟健康行为。核心原则——"打断但不破坏"：使用半透明覆盖层（而非模态弹窗），用户可暂存光标位置，允许"接受提醒并继续工作"。

#### 5.5.3 AI 托管过渡机制

触发强提醒后，AI 自动执行以下过渡流程：

```
1. AI 暂停当前任务上下文 -> 将上下文快照保存至临时文件
2. 显示半透明覆盖层："已连续编码 50+ 分钟，AI 已暂存当前上下文"
3. 用户选择：a) [立即休息 60 秒] -> 倒计时 + 引导休息动作
              b) [推迟 5 分钟] -> 弱化提醒，但间隔缩短至 3 分钟后再次提醒
              c) [跳过本轮] -> 记录跳过事件，计入选从度扣分
4. 休息结束后 -> AI 从快照恢复上下文，提示"欢迎回来，上下文已恢复"
```

关键设计选择：
- **不弹出模态对话框**：使用 terminal overlay/banner 替代，不阻塞终端输入
- **上下文自动保存**：消除用户的"丢失状态"焦虑，降低执行提醒的心理阻力 [DR7]
- **视觉区分**：提醒消息使用 ANSI 颜色区分（柔和蓝色背景），避免与错误/警告混淆

### 5.6 多渠道提醒同步推送架构（设计哲学 4）

当用户通过 Claude Code 安装了 channel 消息渠道（如 WeChat channel、飞书 channel），健康提醒需同步推送到对应 channel，确保即使用户不在终端前也能收到提醒。

#### 5.6.1 Claude Code Channels 机制

Claude Code 的 channel 系统是消息分发的抽象层，允许 skills 通过统一接口将消息路由到多个渠道 [DR4]。核心原理：

```
Skills 层（健康提醒产生）
    |
    v
Claude Code Channel Router（消息路由层）
    |
    +-- Terminal Channel（终端输出，默认）
    +-- WeChat Channel（企业微信/个人微信）
    +-- Feishu Channel（飞书）
    +-- Slack Channel（Slack）
    +-- Custom Webhook Channel（自定义 webhook）
```

Channels 的注册机制：用户执行 `claude channel add <type>` 时，channel 实现注册到路由表。Skills 无需关心具体 channel 实现，只需声明消息的 channel 优先级即可。

#### 5.6.2 健康提醒 Channel 适配层

```
+------------------+     +------------------+     +---------------------------+
|  健康提醒引擎     |---->|  Channel 适配器   |---->|  Claude Code Channel Router|
|  (S5.7)          |     |                  |     +---------------------------+
+------------------+     | .优先级排序      |              |
                         | .内容格式转换    |    +-------+--------+
                         | .隐私脱敏       |    v       v        v
                         +------------------+ Terminal WeChat   Feishu
```

**优先级排序规则**：按用户是否在线自动选择最优渠道：

| 优先级 | 渠道 | 触发条件 | 消息格式 |
|--------|------|---------|---------|
| 1 | Terminal | 终端活跃（心流检测系统判定在线） | ANSI 富文本 |
| 2 | WeChat Channel | WeChat channel 已安装 + 终端无响应 > 2 分钟 | Markdown 卡片 |
| 3 | Feishu Channel | 飞书 channel 已安装 + 终端无响应 > 2 分钟 | 飞书交互卡片 |
| 4 | System Notification | macOS/Windows 原生通知 | 纯文本 |

**内容格式适配**：不同 channel 支持不同的消息格式，适配器负责转换：

| 渠道 | 支持格式 | 转换策略 |
|------|---------|---------|
| Terminal | ANSI color + inline button | 原始格式，无需转换 |
| WeChat | Markdown + 交互卡片 | 降级为 Markdown 文本 + 文本指令 |
| Feishu | 飞书卡片消息（JSON 卡片模板） | 映射到飞书卡片 JSON Schema |
| System | 纯文本 | 去除格式化，保留纯文本指令 |

**隐私保护**：推送至第三方 channel 时自动执行内容脱敏——凯格尔等隐私模块仅推送"是时候了"级别提示，不携带模块名称和详细指引。

#### 5.6.3 同步推送可靠性保障

- **发送确认（ACK）**：每条 channel 推送需收到 channel 层的 ACK 确认，超时 10 秒则降级至下一优先级渠道
- **去重机制**：每条提醒生成唯一 message_id（UUID v7），多 channel 收到同一提醒时仅记录一次"已送达"事件
- **优雅降级**：所有 channel 推送失败时，退化为本地系统通知 + 终端静默记录（下次终端激活时批量提示）

### 5.7 动态自适应提醒引擎（设计哲学 3 + 5）

机械式固定间隔提醒会导致"提醒疲劳"——用户逐渐忽略甚至厌烦提醒 [DR8]。基于 JITAI（Just-in-Time Adaptive Intervention）框架 [DR3]，设计动态自适应引擎，根据用户历史行为实时调整提醒策略。

#### 5.7.1 JITAI 自适应架构

JITAI 框架由四个核心组件构成，本引擎逐一映射 [DR3]：

```
+------------------+     +------------------+     +------------------+
| 决策点检测        |---->| 干预选项选择      |---->| 干预执行          |
| (Decision Point) |     | (Intervention)    |     | (Delivery)       |
+------------------+     +------------------+     +------------------+
        |                                                    |
        |              +------------------+                  |
        +------------->| 适应性评估        |<-----------------+
...
- **决策点检测**：当定时器触发 + 心流状态评估完成时（S5.5.1），进入决策点
- **干预选项选择**：根据用户忠诚度等级选择提醒强度（表 5.7.3）
- **干预执行**：通过多渠道推送架构（S5.6）发送提醒
- **适应性评估**：用户响应后更新忠诚度评分，反馈至下一决策点

#### 5.7.2 用户执行忠诚度评分模型（设计哲学 5）

**评分维度与权重**：

| 评分维度 | 权重 | 计算方式 | 数据来源 |
|---------|------|---------|---------|
| 打卡率（近 7 天） | 0.30 | 实际执行次数 / 应提醒次数 x 100 | 本地 action log |
| 平均响应时间 | 0.20 | 提醒发出到用户确认的平均延迟（秒） | 本地 timestamp delta |
| 连续执行天数 | 0.25 | 至少执行 1 次提醒的连续天数 / 7 | 本地 daily streak |
| 跳过模式 | 0.15 | 连续跳过次数、跳过时段聚集度 | 本地 skip event log |
| 手动触发 | 0.10 | 用户主动触发休息/喝水的频率 | 本地 manual trigger log |

**评分公式**（0-100 分制）：

```
LoyaltyScore = 0.30 x 打卡率得分 
             + 0.20 x (1 - 响应延迟/120秒, 上限1) x 100
             + 0.25 x 连续天数得分
             + 0.15 x (1 - 连续跳过次数/3, 上限1) x 100
             + 0.10 x (手动触发次数/应提醒次数, 上限1) x 100
```
...
| 等级 | 分数范围 | 标签 | 对提醒策略的影响 |
|------|---------|------|---------------|
| L4: 模范执行者 | 85-100 | SSS | 降低频率 20%，推迟按钮延迟上限 +5min |
| L3: 良好执行者 | 65-84 | SS | 保持默认频率，推迟按钮默认参数 |
| L2: 待改善 | 40-64 | S | 加强提醒强度，推迟按钮上限缩短至 3min |
| L1: 流失风险 | 0-39 | -- | 最高提醒强度 + 延长通知持续时间 + 每周汇总报告 |

**激励联动**：

- 忠诚度等级作为打榜权重的独立系数（S4.2）：忠诚度修正后的挑战得分 = 原始桩功消耗 x 忠诚度系数（L4: 1.20x, L3: 1.00x, L2: 0.85x, L1: 0.70x）
- 流失风险用户（L1）触发"回归关怀"：温和鼓励 + 降低目标，避免用户彻底放弃
- 同分场景下打榜排序以忠诚度等级作为 tiebreaker

#### 5.7.3 动态间隔调整算法

根据忠诚度等级和心流状态，动态调节各模块的提醒间隔：

| 模块 | L4 间隔 | L3 间隔（默认） | L2 间隔 | L1 间隔 |
|------|---------|---------------|---------|---------|
| 用眼提醒 | 60 min | 50 min | 45 min | 35 min |
| 久坐提醒 | 40 min | 30 min | 25 min | 20 min |
| 喝水提醒 | 90 min | 75 min | 60 min | 50 min |
| 凯格尔 | 4 次/天 | 3 次/天 | 3 次/天 | 4 次/天 |

调整周期：每 7 天重新评估一次忠诚度等级，避免频繁波动造成用户困惑。等级变更时发送一条摘要通知："本周执行率 X%，等级调整为 Lx，提醒频率已自动优化"。

#### 5.7.4 流失预警与干预策略

基于文献 [DR9] 的健康 App 用户流失研究，识别出以下流失预警信号：

| 预警信号 | 触发条件 | 干预措施 |
|---------|---------|---------|
| 连续跳过 | 连续 3 次跳过同类提醒 | 下次提醒附"需要调整频率吗？"轻度询问 |
| 零执行日 | 全天 0 次执行（非免打扰时段） | 次日早上推送日总结 + 温和建议 |
| 响应延迟增长 | 7 天平均响应时间 > 基线 2x | 自动降低提醒频率至下一级 |
| 模块关闭 | 用户手动 disable 某个模块 | 1 周后推送"最近没在<模块>了"温和回归 |

干预的原则：减少摩擦而非增加压力。所有干预消息使用"建议"语气，不采用警告/批评措辞 [DR8]。
...
### 6.7 设计研究与工程实践来源（DR，共 11 个）

*六项设计哲学的文献支撑，2026-06-08 检索于 Asia/Singapore 时区*

| ID | 来源 | 支持的结论 |
|-----|------|-----------|
| DR1 | Kleitman, N. (1963). *Sleep and Wakefulness*. University of Chicago Press. — 超日节律基础理论 | 90 分钟超日节律的自然注意力周期 |
| DR2 | WU Vienna "60-60-30" 时间管理法 (wu.ac.at, 2024) | 45-50 分钟注意力子周期的实践验证 |
| DR3 | Nahum-Shani et al. (2018). "Just-in-Time Adaptive Interventions (JITAIs) in Mobile Health." *Annals of Behavioral Medicine*, 52(6), 446-462. PMID: 27663578 | 自适应干预四组件（决策点/干预/执行/适应）框架 |
| DR4 | Claude Code Channels — 系统提醒 internal documentation, 2025 | Channel 路由层支持多平台消息同步（Terminal/WeChat/Feishu/Slack 等） |
| DR5 | Mark, G., Gudith, D., & Klocke, U. (2008). "The cost of interrupted work: more speed and stress." *CHI '08*, 107-110. doi:10.1145/1357054.1357072 | 中断后恢复平均需 23 分 15 秒 |
| DR6 | Csikszentmihalyi, M. (1990). *Flow: The Psychology of Optimal Experience*. Harper & Row. | 心流状态的 8 个特征维度与检测指标设计基础 |
| DR7 | Altmann, E. M. & Trafton, J. G. (2002). "Memory for goals: an activation-based model." *Cognitive Science*, 26(1), 39-83. | 任务中断后上下文恢复的心理成本 |
| DR8 | Villinger, K. et al. (2019). "Do we know how to design effective health coaching apps?" *Translational Behavioral Medicine*, 9(4), 683-693. | 提醒疲劳（alert fatigue）的现象、成因与缓解策略 |
| DR9 | Eysenbach, G. (2005). "The law of attrition." *J Med Internet Res*, 7(1), e11. | 健康 App 用户流失曲线：2 周内流失 50%，2 月内 90%  |
| DR10 | Unicode Consortium. (2024). UTR #36: Unicode Security Considerations. unicode.org/reports/tr36/ | Unicode 同形字攻击、全角/半角混淆等安全检测最佳实践 |
| DR11 | Eckersley, P. (2010). "How Unique Is Your Web Browser?" *Privacy Enhancing Technologies*, 1-18. EFF Panopticlick 项目 | 浏览器指纹熵值估算法：~40 bits 足以满足 <100 万用户唯一性 |
...
### Phase 2: 提醒脚本实现（预计 4 周）

| 任务 | 交付物 |
|------|--------|
| 提醒调度引擎 | `scripts/reminder-engine.py`：定时调度、智能抑制、通知分发 |
| 心流检测模块 | `scripts/flow-detector.py`：多维信号采集 + 加权判定（§5.5.1） |
| 自适应提醒控制器 | `scripts/adaptive-controller.py`：忠诚度评分 + 动态间隔调节（§5.7） |
| 久坐提醒脚本 | `scripts/sedentary-reminder.sh`：系统通知、30 分钟间隔、三级递进消息 |
| 用眼提醒脚本 | `scripts/eye-care-reminder.sh`：系统通知、**50 分钟间隔（默认）**、60 秒休息引导 |
| 喝水提醒脚本 | `scripts/hydration-tracker.sh`：进度追踪、75 分钟间隔、日统计 |
| 提肛提醒脚本 | `scripts/kegel-reminder.sh`：每日 2-3 次、隐私文案、分阶段参数 |
| 用户配置解析 | `scripts/config-parser.py`：读取 `~/.vanilla-health/config.yaml`（含动态参数） |
| 平台检测 | `scripts/platform-detector.sh`：macOS/Linux 通知系统适配 |
| Channel 适配器 | `scripts/channel-adapter.py`：多 channel 格式转换 + 推送分发（§5.6） |

### Phase 3: 打榜系统实现（预计 5 周）

| 任务 | 交付物 |
|------|--------|
| Cloudflare Workers 项目初始化 | Wrangler CLI 初始化 + D1 数据库创建 |
| 数据库迁移脚本 | `migrations/001_init.sql`（含 loyalty_score 字段、device_fingerprint 字段） |
| API 实现（register/checkin/leaderboard/stats/challenge/badges） | Workers CRUD 全端点 |
| 昵称合规校验服务 | 多层校验流水线：敏感词库 + LLM 审查 API（§4.9） |
| 设备指纹服务 | 哈希化指纹生成 + 唯一性查重 + 迁移冷却（§4.9.3） |
| 防作弊中间件 | HMAC 签名验证 + 频率限制 + 异常检测 |
| Cloudflare Pages 前端 | 排行榜 + 个人统计 + 忠诚度等级展示 静态站点 |
...
| 平台 | 测试内容 |
|------|---------|
| Claude Code | 安装、description 匹配触发、scripts 执行、通知注入、Channel 推送测试 |
...
### Phase 5: 忠诚度与自适应系统调优（预计 2 周）

| 任务 | 交付物 |
|------|--------|
| 忠诚度评分模型参数校准 | 基于内测数据的权重调优 |
| 动态间隔算法 Beta 验证 | 4 模块 x 4 等级间隔矩阵的可用性测试 |
| 流失预警干预流程测试 | 连续跳过/零执行日/响应延迟增长三种场景模拟 |
| 心流检测阈值调优 | 假阳性/假阴性比例平衡（目标：召回 > 80%，精确 > 70%） |
...
| 超日节律约 90 分钟，注意力子周期约 45-50 分钟 | 2 | DR1, DR2 | 高 |
| 工作中断后恢复平均需约 23 分钟 | 1 | DR5 | 高（经典 CHI 文献，多次复现） |
| JITAI 四组件框架适用于健康提醒自适应 | 1 | DR3 | 高（Annals of Behavioral Medicine 收录） |
| 提醒疲劳现象已被多研究证实 | 1 | DR8 | 高 |
| 健康 App 用户 2 周流失约 50% | 1 | DR9 | 高（J Med Internet Res 里程碑文献） |
| 浏览器/设备指纹 ~40 bits 足以满足百万用户唯一性 | 1 | DR11 | 高（EFF Panopticlick 经典研究） |
